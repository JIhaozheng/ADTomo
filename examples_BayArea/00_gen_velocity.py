"""Generate the background and smooth checkerboard velocity models."""
...
def main():
    ...
    if args.amplitude <= 0 or min(args.wavelength_lon, args.wavelength_lat, args.wavelength_depth) <= 0:
        raise ValueError("amplitude and wavelengths must be positive")

    lon = torch.arange(-125.0, -118.0, 0.04, dtype=torch.float64)
    lat = torch.arange(34.5, 40.0, 0.04, dtype=torch.float64)
    depth = torch.arange(-5.0, 30.1, 3.0, dtype=torch.float64)
