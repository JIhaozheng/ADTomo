...
import eikonal3d_adj_0f_src_op as eikonal3d_op
...
class Eikonal3D(torch.nn.Module):
    def __init__(
        ...
        lambda_dvs=0.0,
        lambda_sp_ratio=0.0,
        beta_dvp=0.0, # damping parameter MODIFIED BY ZIV
        beta_dvs=0.0, # damping parameter MODIFIED BY ZIV
        beta_sp_ratio=0.0, # damping parameter MODIFIED BY ZIV
        lambda_local=0.0,  # NEW: localized smoothing MODIFIED BY ZIV
        config=None, 
        dtype=torch.float64,
    ):
        super().__init__()
        ...
        if inversion not in ("3d", "1d"):
            raise ValueError(f"inversion must be '3d' or '1d', got: {inversion!r}")
        ...
        if inversion == "3d":
            ...
            # 1D inversion: only allow perturbations that vary with depth (z)
            if vp.ndim != 3:
                raise ValueError(f"vp must be a 3D tensor (nx, ny, nz), got shape: {tuple(vp.shape)}")
            nz = vp.shape[-1]
            self.dvp_z = torch.nn.Parameter(torch.zeros((nz,), dtype=vp.dtype, device=vp.device), requires_grad=True)
            self.dvs_z = torch.nn.Parameter(torch.zeros((nz,), dtype=vs.dtype, device=vs.device), requires_grad=True)
            self.dvp = None
            self.dvs = None
        ...
        self.beta_dvp = beta_dvp # damping parameter MODIFIED BY ZIV
        self.beta_dvs = beta_dvs # damping parameter MODIFIED BY ZIV
        self.beta_sp_ratio = beta_sp_ratio  # damping parameter MODIFIED BY ZIV
        self.lambda_local = lambda_local  # NEW: localized smoothing MODIFIED BY ZIV
        ...
        self.register_buffer(
            "smooth_kernel1",
            torch.tensor([1.0, -1.0], dtype=dtype).view(1, 1, 2, 1, 1),
        )
        self.register_buffer(
            "smooth_kernel2",
            torch.tensor([1.0, -1.0], dtype=dtype).view(1, 1, 1, 2, 1),
        )
        self.register_buffer(
            "smooth_kernel3",
            torch.tensor([1.0, -1.0], dtype=dtype).view(1, 1, 1, 1, 2),
        )
        
        #self.smooth_kernel /= self.smooth_kernel.abs().sum()
        # self.smooth_kernel = torch.tensor(
        #     [
        #         [1, -1],
        #         [-1, 1],
    ...
    def _expand_1d_to_3d(self, dz):
        # dz: (nz,) -> (nx, ny, nz)
        return dz.view(1, 1, -1).expand(self.nx, self.ny, self.nz)

    def current_dvp_dvs(self):
        """
        Returns dvp, dvs as 3D tensors (nx, ny, nz) regardless of inversion mode.
        Applies max_dvp/max_dvs bounds if provided.
        """
        if self.inversion == "3d":
            dvp = self.dvp
            dvs = self.dvs
        else:
            dvp = self._expand_1d_to_3d(self.dvp_z)
            dvs = self._expand_1d_to_3d(self.dvs_z)

        if self.max_dvp > 0 or self.max_dvs > 0:
            if self.max_dvp > 0:
                dvp = torch.tanh(dvp) * self.max_dvp
            if self.max_dvs > 0:
                dvs = torch.tanh(dvs) * self.max_dvs
        return dvp, dvs

    def current_vp_vs(self):
        dvp, dvs = self.current_dvp_dvs()
        return self.vp0 + dvp, self.vs0 + dvs
    ...
    def forward(self, picks):
        ...
        for (idx_sta_, phase_type_), picks_ in picks.groupby(["idx_sta", "phase_type"]):
            ...
            if phase_type_ == "P":
                ...
                res = at - torch.tensor(picks_["phase_time"].values, dtype=self.dtype)
                loss += 0.5 * (res * res).sum()
        ...
        if self.lambda_dvp > 0:
            x = dvp.unsqueeze(0).unsqueeze(0)
            g1 = F.conv3d(x, self.smooth_kernel1, padding=0)
            g2 = F.conv3d(x, self.smooth_kernel2, padding=0)
            g3 = F.conv3d(x, self.smooth_kernel3, padding=0)

            loss += self.lambda_dvp * (g1.pow(2).sum() + g2.pow(2).sum() + g3.pow(2).sum())

        if self.lambda_dvs > 0:
            x = dvs.unsqueeze(0).unsqueeze(0)
            g1 = F.conv3d(x, self.smooth_kernel1, padding=0)
            g2 = F.conv3d(x, self.smooth_kernel2, padding=0)
            g3 = F.conv3d(x, self.smooth_kernel3, padding=0)
            loss += self.lambda_dvs * (g1.pow(2).sum() + g2.pow(2).sum() + g3.pow(2).sum())
 
        if self.lambda_sp_ratio > 0:
            sp_ratio = vs / vp
            x= sp_ratio.unsqueeze(0).unsqueeze(0)
            g1 = F.conv3d(x, self.smooth_kernel1, padding=0)
            g2 = F.conv3d(x, self.smooth_kernel2, padding=0)
            g3 = F.conv3d(x, self.smooth_kernel3, padding=0)
            reg_sp_ratio = g1.pow(2).sum() + g2.pow(2).sum() + g3.pow(2).sum()
            loss += self.lambda_sp_ratio * reg_sp_ratio.pow(2).sum()
            
        # damping regularization MODIFIED BY ZIV
        if self.beta_dvp > 0:
            loss += self.beta_dvp * dvp.pow(2).sum()
        if self.beta_dvs > 0:
            loss += self.beta_dvs * dvs.pow(2).sum()
        if self.beta_sp_ratio > 0:
            sp_ratio = vp / vs - 1.732
            loss += self.beta_sp_ratio * sp_ratio.pow(2).sum()
        
        # =====================================================
        # Extra localized smoothing near stations and events
        # MODIFIED BY ZIV 10/01/2025
        # =====================================================
        if self.lambda_local > 0:
            neighbors = []
            # collect station and event positions in grid coordinates
            all_points = []
            all_points.append(self.station_loc.weight.detach() / self.h)  # (num_sta, 3)
            all_points.append(self.event_loc.weight.detach() / self.h)    # (num_evt, 3)
            all_points = torch.cat(all_points, dim=0)

            for pt in all_points:
                ix = int(torch.clamp(torch.round(pt[0]), 0, self.nx - 2))
                iy = int(torch.clamp(torch.round(pt[1]), 0, self.ny - 2))
                iz = int(torch.clamp(torch.round(pt[2]), 0, self.nz - 2))
                # nearest 8 cube vertices
                cube = [
                    (ix, iy, iz),
                    (ix+1, iy, iz),
                    (ix, iy+1, iz),
                ...
                neighbors.extend(cube)

            # remove duplicates
            neighbors = list(set(neighbors))

            # apply local smoothing penalty across those neighbors
            for (ix, iy, iz) in neighbors:
                if ix < self.nx and iy < self.ny and iz < self.nz:
                    if ix+1 < self.nx:
                        loss += self.lambda_local * (dvp[ix,iy,iz] - dvp[ix+1,iy,iz]).pow(2)
                        loss += self.lambda_local * (dvs[ix,iy,iz] - dvs[ix+1,iy,iz]).pow(2)
                    if iy+1 < self.ny:
                        loss += self.lambda_local * (dvp[ix,iy,iz] - dvp[ix,iy+1,iz]).pow(2)
                        loss += self.lambda_local * (dvs[ix,iy,iz] - dvs[ix,iy+1,iz]).pow(2)
                    if iz+1 < self.nz:
                        loss += self.lambda_local * (dvp[ix,iy,iz] - dvp[ix,iy,iz+1]).pow(2)
                        loss += self.lambda_local * (dvs[ix,iy,iz] - dvs[ix,iy,iz+1]).pow(2)
