// 2D continuous adjoint: zero-flux FSM + source-box ∇T (adjderivonsource).
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("backward", &eikonal_backward,
          "Eikonal2D backward (zero-flux FSM, nabla T, no src corner overwrite)");
    m.def("solve_adjoint", &eikonal_solve_adjoint,
          "Eikonal2D adjoint solve: nabla·(lambda nabla T)=delta, zero-flux BC + source nabla T");
}
