...
static void zero_adjoint_boundaries(double *lam, int nx, int ny) {
    for (int i = 0; i < nx; ++i) {
        lam[gid(i, 0, ny)] = 0.0;
        lam[gid(i, ny - 1, ny)] = 0.0;
    }
    for (int j = 0; j < ny; ++j) {
        lam[gid(0, j, ny)] = 0.0;
        lam[gid(nx - 1, j, ny)] = 0.0;
    }
}

static void solve_adjoint_fsm_dir(double *lam, const double *T, const double *delta,
            ...
            for (int sy_d : {1, -1}) {
                ...
                for (int i = std::get<0>(I); i != std::get<1>(I); i += std::get<2>(I))
                    for (int j = std::get<0>(J); j != std::get<1>(J); j += std::get<2>(J))
                        lam[gid(i, j, ny)] =
                            adjoint_stencil_dir(T, lam, delta, nx, ny, i, j, h, sx, sy);
            }
        zero_adjoint_boundaries(lam, nx, ny);
        double err = 0.0;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("backward", &eikonal_backward,
          "Eikonal2D backward (Dirichlet FSM, nabla T, no src corner overwrite)");
}
