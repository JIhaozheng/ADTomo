...
// Normal-projected ∇(-T) toward source at source-box nodes.
...
static void sweep_adjoint_0f(double *lam, const double *T, const double *delta,
    ...
    for (int i = std::get<0>(I); i != std::get<1>(I); i += std::get<2>(I))
        for (int j = std::get<0>(J); j != std::get<1>(J); j += std::get<2>(J))
            for (int k = std::get<0>(K); k != std::get<1>(K); k += std::get<2>(K))
                lam[gid(i, j, k, n, l)] =
                    adjoint_stencil_0f(T, lam, delta, m, n, l, i, j, k, h, sx, sy, sz);
}

static void solve_adjoint_fsm_0f(double *lambda, const double *T, const double *delta,
                                 int m, int n, int l, double h,
                                 double sx, double sy, double sz,
    ...
    for (int it = 0; it < max_iter; ++it) {
        ...
        sweep_adjoint_0f(lambda, T, delta, m, n, l, h, sx, sy, sz, 1, 1, 1);
        sweep_adjoint_0f(lambda, T, delta, m, n, l, h, sx, sy, sz, -1, 1, 1);
        sweep_adjoint_0f(lambda, T, delta, m, n, l, h, sx, sy, sz, -1, -1, 1);
        sweep_adjoint_0f(lambda, T, delta, m, n, l, h, sx, sy, sz, 1, -1, 1);
        sweep_adjoint_0f(lambda, T, delta, m, n, l, h, sx, sy, sz, 1, -1, -1);
...
torch::Tensor eikonal_solve_adjoint(torch::Tensor T, torch::Tensor delta, double h,
    ...
    solve_adjoint_fsm_0f(lambda.data_ptr<double>(), T.data_ptr<double>(),
                         delta.data_ptr<double>(), m, n, l, h, x, y, z);
    return lambda;
}
