"""Invert the synthetic 1-D dataset: Vp(z)/Vs(z) and/or event locations and origin times.

Choose what is trainable, as in AI4EPS/ADTomo (parameters are toggled with
``requires_grad``):

    python inversion1d.py --trainable vp,vs
    python inversion1d.py --trainable event_loc,event_time
    python inversion1d.py --trainable vp,vs,event_loc,event_time
    torchrun --standalone --nproc_per_node=4 inversion1d.py --trainable vp,vs
...
from adtomo import ForwardGrid2D, Tomography2D, VelocityModel1D
...
def parse_args():
    ...
    parser.add_argument("--grid-padding", type=float, default=10.0, help="margin (km) around the initial events so relocated events stay inside the fixed section")
...
def prepare_station_groups(stations, events_initial, picks, model, spacing, padding, rank, world_size):
...
def plot_velocity(true, initial, model, path):
    ...
    for phase, color in (("vp", "tab:red"), ("vs", "tab:blue")):
        axis.plot(true[phase], model.depth, "-", color=color, label=f"{phase.upper()} true")
        axis.plot(initial[phase], model.depth, ":", color=color, label=f"{phase.upper()} initial")
...
def main():
    ...
    try:
        ...
        model = VelocityModel1D(**initial, trainable=True)
        tomography = Tomography2D(
            model, events_initial[["longitude", "latitude", "depth_km"]].to_numpy(),
            args.lambda_vp, args.lambda_vs, args.alpha_vp, args.alpha_vs,
        )
        parameters = configure_trainable(tomography, args.trainable)
        groups = prepare_station_groups(stations, events_initial, picks, model, args.spacing, args.grid_padding, rank, world_size)
        if not groups:
        ...
        if rank == 0:
            ...
            torch.save(
                {"depth": model.depth, "vp": model.vp.detach(), "vs": model.vs.detach(), "loss_history": history},
