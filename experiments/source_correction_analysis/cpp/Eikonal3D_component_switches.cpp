...
static void patch_lu_corner_res_3d(
    ...
    Eigen::MatrixXd *matrix_out = nullptr) {
    const int nn = m * n * l;
    int ix0 = std::max(0, std::min((int)std::floor(x), m - 1));
    int jx0 = std::max(0, std::min((int)std::floor(y), n - 1));
    int kx0 = std::max(0, std::min((int)std::floor(z), l - 1));
...
// With pinned source rows F_C = T_C - S(p_C), the local matrix is I.
// Accumulate the exact discrete transpose coupling from ordinary Godunov rows:
// lambda_C = g_C - F_EC^T lambda_E.
static void compute_source_corner_adjoint_explicit(
    double *lambda_source, const double *grad_u, const double *u, const double *lambda_scaled,
    int m, int n, int l, double x, double y, double z, int radius = 1) {
    const int nn = m * n * l;
    int ix0 = std::max(0, std::min((int)std::floor(x), m - 1)); int ix1 = std::min(ix0 + 1, m - 1);
    int jx0 = std::max(0, std::min((int)std::floor(y), n - 1)); int jx1 = std::min(jx0 + 1, n - 1);
    int kx0 = std::max(0, std::min((int)std::floor(z), l - 1)); int kx1 = std::min(kx0 + 1, l - 1);
    std::memcpy(lambda_source, lambda_scaled, sizeof(double) * nn);
    std::unordered_map<int, bool> is_corner;
    for (int i : {ix0, ix1}) for (int j : {jx0, jx1}) for (int k : {kx0, kx1}) {
        const int id = gid(i, j, k, n, l); is_corner[id] = true; lambda_source[id] = grad_u[id];
    }
    std::vector<std::pair<int, double>> entries;
    for (int i = std::max(0, ix0-radius); i <= std::min(m-1, ix1+radius); ++i)
        for (int j = std::max(0, jx0-radius); j <= std::min(n-1, jx1+radius); ++j)
            for (int k = std::max(0, kx0-radius); k <= std::min(l-1, kx1+radius); ++k) {
                const int row = gid(i,j,k,n,l);
                if (is_corner.count(row)) continue;
                collect_godunov_row(entries, u, m,n,l,i,j,k,ix0,jx0,kx0,ix1,jx1,kx1);
                for (const auto &entry : entries)
                    if (is_corner.count(entry.first)) lambda_source[entry.first] -= entry.second * lambda_scaled[row];
            }
}

// Hybrid backward: FSM bulk + src correction on source corners only.
// Experiment-only decomposition of the production backward.  LOCAL_LU only
// produces the residual consumed by SOURCE_GRADIENT, so L without G is not a
// meaningful operator and is rejected at the binding boundary.
static void backward_components(
    double *grad_f, const double *grad_u, const double *u, const double *f, double h,
    int m, int n, int l, double x, double y, double z,
    bool source_stencil, bool local_lu, bool source_gradient) {
    const int nn = m * n * l;
    ...
    solve_adjoint_fsm_0f_src(lambda.data(), u, delta.data(), m, n, l, h, x, y, z, source_stencil);

    for (int i = 0; i < nn; ++i) grad_f[i] = lambda[i] * f[i] * vol;
    if (!source_gradient) return;
    ...
    if (local_lu) {
        ...
        // Direct Simpson derivative driven by the scaled continuous adjoint.
        apply_source_simpson_grad_from_res(grad_f, lam_scaled.data(), m, n, l, h, x, y, z);
    }
}
...
torch::Tensor eikonal_backward(torch::Tensor grad_u, torch::Tensor u, torch::Tensor f,
    ...
    backward_components(grad_f.data_ptr<double>(), grad_u.data_ptr<double>(), u.data_ptr<double>(),
                        f.data_ptr<double>(), h, m, n, l, x, y, z, true, true, true);
    return grad_f;
}

torch::Tensor eikonal_backward_components(torch::Tensor grad_u, torch::Tensor u, torch::Tensor f,
                                          double h, double x, double y, double z,
                                          bool source_stencil, bool local_lu, bool source_gradient) {
    TORCH_CHECK(!local_lu || source_gradient, "LOCAL_LU requires SOURCE_GRADIENT");
    TORCH_CHECK(grad_u.dim() == 3 && u.dim() == 3 && f.dim() == 3, "All tensors must be 3D");
    TORCH_CHECK(grad_u.sizes() == u.sizes() && u.sizes() == f.sizes(), "All tensors must have the same size");
    TORCH_CHECK(grad_u.is_contiguous() && u.is_contiguous() && f.is_contiguous(), "All tensors must be contiguous");
    auto grad_f = torch::zeros_like(f);
    backward_components(grad_f.data_ptr<double>(), grad_u.data_ptr<double>(), u.data_ptr<double>(),
                        f.data_ptr<double>(), h, u.size(0), u.size(1), u.size(2), x, y, z,
                        source_stencil, local_lu, source_gradient);
    return grad_f;
}

torch::Tensor eikonal_local_source_matrix(torch::Tensor u, double x, double y, double z) {
    TORCH_CHECK(u.dim() == 3 && u.is_contiguous(), "u must be a contiguous 3D tensor");
    auto scratch = torch::zeros_like(u);
    Eigen::MatrixXd A;
    patch_lu_corner_res_3d(scratch.data_ptr<double>(), scratch.data_ptr<double>(), u.data_ptr<double>(),
                           scratch.data_ptr<double>(), u.size(0), u.size(1), u.size(2), x, y, z, 1, &A);
    auto out = torch::zeros({A.rows(), A.cols()}, u.options());
    for (int i = 0; i < A.rows(); ++i)
        for (int j = 0; j < A.cols(); ++j)
            out.data_ptr<double>()[i * A.cols() + j] = A(i, j);
    return out;
}

torch::Tensor eikonal_backward_explicit(torch::Tensor grad_u, torch::Tensor u, torch::Tensor f,
                                        double h, double x, double y, double z) {
    const int nn = u.numel(); const double vol = h*h*h, lam_scale = 0.5*h;
    std::vector<double> delta(nn), lambda(nn), lambda_scaled(nn), lambda_source(nn);
    for (int i=0;i<nn;++i) delta[i]=grad_u.data_ptr<double>()[i]/vol;
    solve_adjoint_fsm_0f_src(lambda.data(),u.data_ptr<double>(),delta.data(),u.size(0),u.size(1),u.size(2),h,x,y,z,false);
    auto out=torch::zeros_like(f);
    for (int i=0;i<nn;++i) { out.data_ptr<double>()[i]=lambda[i]*f.data_ptr<double>()[i]*vol; lambda_scaled[i]=lambda[i]*lam_scale; }
    compute_source_corner_adjoint_explicit(lambda_source.data(),grad_u.data_ptr<double>(),u.data_ptr<double>(),lambda_scaled.data(),u.size(0),u.size(1),u.size(2),x,y,z);
    apply_source_simpson_grad_from_res(out.data_ptr<double>(),lambda_source.data(),u.size(0),u.size(1),u.size(2),h,x,y,z);
    return out;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("backward", &eikonal_backward, "3D zero-flux FSM adjoint + src correction");
    m.def("backward_components", &eikonal_backward_components, "3D experiment-only component-switched backward");
    m.def("local_source_matrix", &eikonal_local_source_matrix, "3D local source-corner adjoint matrix");
    m.def("backward_explicit", &eikonal_backward_explicit, "3D explicit source-corner discrete adjoint");
    m.def("solve_adjoint", &eikonal_solve_adjoint, "3D zero-flux FSM adjoint + src correction",
