...
local_velocity = torch.full((4, 5, 6), 5.0, dtype=torch.float64, requires_grad=True)
local_traveltime = solve_eikonal3d(local_velocity, (1.2, 1.3, 1.1), 1.0)
local_traveltime[3, 4, 5].backward()
assert local_velocity.grad is not None
assert torch.isfinite(local_velocity.grad).all()
assert local_velocity.grad.abs().sum() > 0
...
for epsilon in (1e-2, 1e-3):
    perturbed_traveltime = solve_eikonal3d(
        taylor_velocity.detach() + epsilon * taylor_direction, (1.2, 1.3, 1.1), 1.0
    )[3, 4, 5]
    kernel_remainders.append(abs(perturbed_traveltime.item() - taylor_objective.item() - epsilon * taylor_derivative))
...
gradient_grid = ForwardGrid(station_lonlatdepth, event_lonlatdepth, model, spacing=5.0)
predicted_p_phase_dt = predict_phase_times(model, gradient_grid, "P", torch.zeros(1, dtype=torch.float64))
predicted_s_phase_dt = predict_phase_times(model, gradient_grid, "S", torch.zeros(1, dtype=torch.float64))
gradient_loss = (predicted_p_phase_dt - 3.0).square().mean() + (predicted_s_phase_dt - 5.0).square().mean()
...
for epsilon in epsilons:
    perturbed_model = VelocityModel(
        lon,
        lat,
        depth,
        base_vp + epsilon * global_direction,
        vp / 1.73,
        trainable=False,
    )
...
reference_remainder = full_remainders[0] * (epsilons_tensor / epsilons_tensor[0]).square()
figure, axis = plt.subplots(figsize=(5, 4), constrained_layout=True)
axis.loglog(epsilons, full_remainders, "o-", label="Full-chain remainder")
axis.loglog(epsilons, reference_remainder, "--", label=r"$O(\epsilon^2)$")
axis.set(xlabel=r"$\epsilon$", ylabel="Taylor remainder", title="Global Vp Taylor test")
